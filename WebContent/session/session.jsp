...
<body>
<div>http 프로토콜은 statelss(상태정보 유지안됨) ,비연결지향  <br>
    (참고로 카카오톡은 연결지향.)
     ...
     -> 또는 session.setMaxinactiveInterval(300); 로 합니다. <br>
</div>
<!-- 
    <session-config>
...
<%
	...
	long create = session.getCreationTime();	//세션객체가 만들어진 시간(첫번째 요청)
	...
	long last = session.getLastAccessedTime();	//최근 요청시간(마지막 요청)
