...
<ul>
	...
	<li>el 은 expression language. 기호는 $&#123;&#125; 사용합니다.
	<%-- <%= %> --%> 기호 대신 사용합니다. 애트리뷰트 getAttribute 메소드 동작 실행 후 출력</li>
	<li>jstl 과 el은 함께 사용됩니다. (el 은 라이브러리 추가 없이 사용합니다.)</li>
</ul>
<h4>자바 변수 처럼 값을 저장하기</h4>
...
<c:if test="${age < 20 }">
	<div style="color:green;">청소년  입니다.</div></c:if>
<c:if test="${age >= 20 }">
	<div style="color:blue;">성인  입니다.</div></c:if>	
...
<c:forEach var="x" begin="10" end="30" step="2" varStatus="status"> <!-- varStatus 는 현재상태 저장할 변수 -->
	...
	<c:out value="${status.count }"/> <br> <!-- varStatus의 count 속성은 1부터 시작하여 몇번째 값인지  -->
</c:forEach>
